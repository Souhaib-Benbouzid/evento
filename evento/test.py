...
class Party_Invitees_Tests(unittest.TestCase):
    ...
    def test_get_invitee_details(self):
        """TEST: Host add party  """
        res = self.client().get('/')
    ...
    # party invitee interatction tests

    def test_get_list_of_invitee_in_party(self):
        res = self.client().get('/')

        self.assertEqual(res.status_code, 200)

    def test_add_invitee_to_party(self):
        """TEST: Host add party  """
        res = self.client().get('/')
    ...
    def test_delete_invitee_from_party(self):
        """TEST: Host add party  """
        res = self.client().get('/')
